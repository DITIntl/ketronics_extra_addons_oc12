...
class ButirSalesReportWizard(models.TransientModel):
    ...
    filter = fields.Selection(string="Filter", selection=[('with_faktur', 'With faktur pajak'), ('without_faktur', 'Without faktur pajak'), ], required=True, )
    ...
    def get_ppn_idr(self, lines):
        ...
        for inv in usd_invoices:
            if (inv.partner_id.is_base_statement):
                inv_year = inv.date_invoice.year
                inv_month = inv.date_invoice.month
        ...
        #     for tax in line.invoice_line_tax_ids:
        #         if tax.id == default_sales_tax.id:
        #             if (line.invoice_id.partner_id.is_base_statement):
        #                 inv_year = line.date_invoice.year
        #                 inv_month = line.date_invoice.month
        #                 last_day_of_month = calendar.monthrange(inv_year, inv_month)[1]
        #                 invoice_date = datetime.date(
        #                     inv_year, inv_month, last_day_of_month)
        #             else:
        ...
        #                 tax_line_usd, idr_curr, self.env.user.company_id, invoice_date)
        #             # print(tax_line_idr )
    ...
    def get_dpp_idr(self, lines):
        ...
        #         line.price_unit, idr_curr, self.env.user.company_id, invoice_date)
