	...
	def action_apply(self):
		...
		if picking_ids:
			for picking in picking_ids.filtered(lambda x: x.state not in ('cancel')):
				picking.write({'scheduled_date': self.transfer_date, 'date_done': self.transfer_date})
				for data in picking.move_lines:
					data.write({'date': self.transfer_date, 'reference': picking.name, 'remark': self.remark, 'transfer_date': self.transfer_date})
					for line in data.mapped('move_line_ids'):
						line.write({'date': self.transfer_date or fields.Datetime.now(), 'reference': picking.name})
